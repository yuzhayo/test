...
export function compose(
  wrappers: Array<"spin" | "orbit" | "pulse" | "fade">
) {
  ...
  return (input: LibraryConfig, ctx: ProcessingContext): { layers: LayerData[]; warnings: string[] } => {
    const { stage, layers, warnings } = normalizeInput(input);
    const out = layers.map((norm, idx) =>
      buildLayerData(norm, idx, ctx, opts, stage)
    );
