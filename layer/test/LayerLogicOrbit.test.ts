...
describe('LayerLogicOrbit', () => {
  describe('applyOrbit', () => {
    it('should return original position when disabled', () => {
      ...
      const result = applyOrbit({ position: { x: 10, y: 20 } }, config, 1, { x: 0, y: 0 });
    ...
    it('should calculate clockwise orbit correctly at time 0', () => {
      ...
      const result = applyOrbit({ position: { x: 0, y: 0 } }, config, 0, baseCenter);
    ...
    it('should calculate quarter rotation correctly', () => {
      ...
      const result = applyOrbit({ position: { x: 0, y: 0 } }, config, 0.25, baseCenter);
    ...
    it('should calculate half rotation correctly', () => {
      ...
      const result = applyOrbit({ position: { x: 0, y: 0 } }, config, 0.5, baseCenter);
    ...
    it('should handle invalid time values', () => {
      ...
      expect(applyOrbit({ position: originalPos }, config, NaN, baseCenter).position)
        .toEqual(originalPos);
      expect(applyOrbit({ position: originalPos }, config, Infinity, baseCenter).position)
        .toEqual(originalPos);
    });
  });
});