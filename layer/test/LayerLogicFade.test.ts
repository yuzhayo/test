...
describe('LayerLogicFade', () => {
  describe('applyFade', () => {
    it('should return original opacity when disabled', () => {
      const config: FadeConfig = { enabled: false, from: 0.2, to: 0.8, rpm: 30 };
    ...
    it('should clamp values to 0-1 range', () => {
      const config: FadeConfig = { enabled: true, from: -0.5, to: 1.5, rpm: 60 };
    ...
    it('should handle invalid time values', () => {
      ...
      expect(applyFade({ opacity: originalOpacity }, config, NaN).opacity)
        .toBe(originalOpacity);
      expect(applyFade({ opacity: originalOpacity }, config, Infinity).opacity)
        .toBe(originalOpacity);
    });
