...
describe('LayerPipeline', () => {
  ...
  describe('produceBasicSpin', () => {
    it('should apply only spin animation', () => {
      const result = produceBasicSpin(basicConfig, { ...mockContext, time: 0.5 });
      expect(result.layers.length).toBeGreaterThan(0); const layer = result.layers[0]!;
  ...
  describe('produceBasicOrbit', () => {
    it('should apply only orbit animation', () => {
      const result = produceBasicOrbit(basicConfig, { ...mockContext, time: 0.5 });
      expect(result.layers.length).toBeGreaterThan(0); const layer = result.layers[0]!;
  ...
  describe('produceBasicPulse', () => {
    it('should apply only pulse animation', () => {
      const result = produceBasicPulse(basicConfig, { ...mockContext, time: (0.25 / 45) * 60 }); // adjusted for 45 RPM
      expect(result.layers.length).toBeGreaterThan(0); const layer = result.layers[0]!;
  ...
  describe('produceBasicFade', () => {
    it('should apply only fade animation', () => {
      const result = produceBasicFade(basicConfig, { ...mockContext, time: (0.25 / 40) * 60 }); // adjusted for 40 RPM
      expect(result.layers.length).toBeGreaterThan(0); const layer = result.layers[0]!;
  ...
  describe('error handling', () => {
    it('should throw error for invalid config', () => {
      ...
      expect(() => produceBasic(invalidConfig, mockContext)).toThrow('validation failed');
      expect(() => produceBasicSpin(invalidConfig, mockContext)).toThrow('validation failed');
      expect(() => compose(['spin'])(invalidConfig, mockContext)).toThrow('validation failed');
    });
