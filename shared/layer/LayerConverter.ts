...
export interface UILayerControls {
  ...
  spin: Pick<BehaviorSpinConfig, "enabled" | "rpm" | "direction">;
  orbit: Pick<BehaviorOrbitConfig, "enabled" | "rpm" | "radius">;
  pulse: Pick<BehaviorPulseConfig, "enabled" | "amplitude" | "rpm">;
  fade: Pick<BehaviorFadeConfig, "enabled" | "from" | "to" | "rpm">;
}
...
export function fromUILayerControls(
  ...
  const orbit: BehaviorOrbitConfig = {
  ...
  const pulse: BehaviorPulseConfig = {
    enabled: ui.pulse?.enabled ?? base.behaviors.pulse.enabled,
    amplitude: ui.pulse?.amplitude !== undefined ? ui.pulse.amplitude : base.behaviors.pulse.amplitude,
    rpm: ui.pulse?.rpm !== undefined ? fromUIRpm(ui.pulse.rpm, rpmOpts) : base.behaviors.pulse.rpm,
  };

  const fade: BehaviorFadeConfig = {
    enabled: ui.fade?.enabled ?? base.behaviors.fade.enabled,
    from: ui.fade?.from !== undefined ? fromUIOpacity(ui.fade.from, { min: 0, max: 1 }) : base.behaviors.fade.from,
    to: ui.fade?.to !== undefined ? fromUIOpacity(ui.fade.to, { min: 0, max: 1 }) : base.behaviors.fade.to,
