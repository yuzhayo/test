...
const toAssetRef = (layer: LayerConfig): AssetRef | null => {
  ...
  return hasPath ? { type: "path", path: layer.imagePath! } : { type: "registry", key: layer.registryKey! };
};
...
const normalizeBehaviors = (b?: BehaviorsConfig): BehaviorsConfigNormalized => ({
  ...
  pulse: {
    ...
    amplitude: isNum(b?.pulse?.amplitude) ? (b!.pulse!.amplitude as number) : DEFAULT_BEHAVIORS.pulse.amplitude,
...
function validateEvents(
  ...
  for (const k of keys) {
    ...
    if (!Array.isArray(list)) {
      errors.push({ code: "events.invalid", path: `${basePath}.${k}`, message: "must be an array", severity: "error" });
      continue;
    }
    ...
    list.forEach((entry, idx) => {
      ...
      if (set && typeof set !== "object") {
        errors.push({ code: "events.set.invalid", path, message: "set must be an object", severity: "error" });
        return;
      }
      // light checks per action (numeric/boolean shape)
      if (set?.rpm !== undefined && !isNum(set.rpm)) {
        errors.push({ code: "events.set.rpm", path, message: "rpm must be number", severity: "error" });
      }
      if (set?.enabled !== undefined && typeof set.enabled !== "boolean") {
        errors.push({ code: "events.set.enabled", path, message: "enabled must be boolean", severity: "error" });
      }
      if (action === "spin" && set?.direction && set.direction !== "cw" && set.direction !== "ccw") {
        errors.push({ code: "events.set.direction", path, message: 'direction must be "cw"|"ccw"', severity: "error" });
      }
      if (action === "orbit") {
        if (set?.radius !== undefined && !isNum(set.radius)) {
          errors.push({ code: "events.set.radius", path, message: "radius must be number", severity: "error" });
        }
        if (set?.center !== undefined && !isVec2(set.center)) {
          errors.push({ code: "events.set.center", path, message: "center must be {x,y}", severity: "error" });
        }
      }
      if (action === "pulse" && set?.amplitude !== undefined && !isNum(set.amplitude)) {
        errors.push({ code: "events.set.amplitude", path, message: "amplitude must be number", severity: "error" });
      }
      if (action === "fade") {
        if (set?.from !== undefined && !isNum(set.from)) {
          errors.push({ code: "events.set.from", path, message: "from must be number", severity: "error" });
        }
        if (set?.to !== undefined && !isNum(set.to)) {
          errors.push({ code: "events.set.to", path, message: "to must be number", severity: "error" });
        }
      }
...
export function validateLibraryConfig(input: LibraryConfig): ValidationResult {
  ...
  const stage: StageConfigNormalized = {
    width: isNum(input.stage?.width) && input.stage!.width! > 0 ? input.stage!.width! : DEFAULT_STAGE.width,
    height: isNum(input.stage?.height) && input.stage!.height! > 0 ? input.stage!.height! : DEFAULT_STAGE.height,
  ...
  if (!Array.isArray(input.layers)) {
    errors.push({ code: "layers.missing", path: "layers", message: "layers must be an array", severity: "error" });
  ...
  input.layers.forEach((layer, idx) => {
    ...
    // id
    if (!layer || typeof layer !== "object" || typeof layer.layerId !== "string" || layer.layerId.length === 0) {
      errors.push({ code: "layer.id.missing", path: `${base}.layerId`, message: "layerId required", severity: "error" });
      return;
    }
    if (seen.has(layer.layerId)) {
      errors.push({ code: "layer.id.duplicate", path: `${base}.layerId`, message: `duplicate "${layer.layerId}"`, severity: "error" });
      return;
    }
    ...
    if (!assetRef) {
      ...
      errors.push({
        ...
        path: base,
        message: both ? "imagePath and registryKey are mutually exclusive" : "one of imagePath or registryKey is required",
        severity: "error",
      });
      return;
    }
    ...
    if (opacity < 0 || opacity > 1) {
      warnings.push({ code: "layer.opacity.range", path: `${base}.opacity`, message: "opacity should be within [0,1]", severity: "warning" });
    }
    if (anchor.x < 0 || anchor.x > 1 || anchor.y < 0 || anchor.y > 1) {
      warnings.push({ code: "layer.anchor.range", path: `${base}.anchor`, message: "anchor should be [0..1]", severity: "warning" });
    }
    ...
    if (fitMode !== undefined && !isFitMode(fitMode)) {
      errors.push({ code: "layer.fit.invalid", path: `${base}.fitMode`, message: "fitMode must be contain|cover|stretch", severity: "error" });
    }
    if (alignment !== undefined && !isAlignment(alignment)) {
      errors.push({ code: "layer.alignment.invalid", path: `${base}.alignment`, message: "invalid alignment", severity: "error" });
    }
    ...
    if (behaviors.orbit.radius < 0 || behaviors.orbit.rpm < 0) {
      warnings.push({ code: "behavior.orbit.range", path: `${base}.behaviors.orbit`, message: "rpm & radius should be >= 0", severity: "warning" });
    }
    if (behaviors.spin.rpm < 0) {
      warnings.push({ code: "behavior.spin.range", path: `${base}.behaviors.spin.rpm`, message: "rpm should be >= 0", severity: "warning" });
    }
    if (behaviors.pulse.rpm < 0 || behaviors.pulse.amplitude < 0) {
      warnings.push({ code: "behavior.pulse.range", path: `${base}.behaviors.pulse`, message: "rpm & amplitude should be >= 0", severity: "warning" });
    }
    if (behaviors.fade.rpm < 0) {
      warnings.push({ code: "behavior.fade.range", path: `${base}.behaviors.fade.rpm`, message: "rpm should be >= 0", severity: "warning" });
    }
    ...
    const normalized: LayerConfigNormalized = {
      ...
      container:
        width || height || fitMode || alignment
          ? { width, height, fitMode: fitMode as FitMode, alignment: alignment as Alignment }
          : undefined,
    };
  ...
  return ok
    ? { ok: true, errors, warnings, normalized: { stage, layers: normalizedLayers } }
